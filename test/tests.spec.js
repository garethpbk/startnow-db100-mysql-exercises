...
const readFile = file =>
  new Promise((resolve, reject) => {
    fs.readFile(path.join(__dirname, file), "utf-8", (err, data) => (err ? reject(err) : resolve(data)));
  });
...
const readAndExecute = (filename, callback) => readFile(path.join("../sql", `${filename}.sql`)).then(executeSql);

const readAndExecuteSequential = (...files) =>
  files.reduce((promise, file) => promise.then(() => readAndExecute(file)), Promise.resolve());
...
describe("SQL Exercises -", function() {
  ...
  describe("JOINS -", () => {
    ...
    it("9d. Select city and country name, using a LEFT join on country table", () => {
      gradeExercise(results[32], 600, "city", "country", "country_id");
    });
    it("9e. Select films with language info, using a LEFT join on language table", () => {
      gradeExercise(results[33], 1000, "title", "description", "language_id", "release_year", "language");
    });
